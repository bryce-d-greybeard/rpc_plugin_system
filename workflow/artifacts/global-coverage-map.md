...
| `provider-observability` | event schema unit tests, SDK emission tests, admin/CLI log-read tests, redaction negative tests, downstream provider logging docs | `provider-observability.event-schema` implemented with `src/internal/eventlog` tests and evidence at `workflow/features/provider-observability.event-schema/evidence/implementation-2026-05-23.md`; next active slice is `provider-observability.sdk-emission` | no accepted gaps for event-schema; payload/secret/authority-handle logging is explicitly out of scope and is rejected by tests |
