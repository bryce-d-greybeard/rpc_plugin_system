...
import (
	...
	"rpc_plugin_system/secure-bootstrap-transport/startup-handshake/auth"
)
...
func buildFailurePlugin(t *testing.T) string {
	...
	cmd := exec.Command("go", "build", "-buildvcs=false", "-o", failurePluginBuildPath, "./plugin-authoring-sdk/failure-example/rpcplugin-failure")
