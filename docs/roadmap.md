...
Goal:
- make the local trust and failure model much harder to break
...
Goal:
- add cleaner external/operator-facing transport options for dashboards, admin surfaces, and browser-oriented tooling without confusing them with the plugin transport itself

Direction:
- keep UI/control-plane transport separate from plugin transport
- use UI-facing transports for operator tools, dashboards, and browser integration
